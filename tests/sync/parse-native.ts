...
const test = suite('sync/parse-native');
...
test('should return an object', () => {
	assert.type(parseNative('str'), 'object');
});

test('should reject for invalid filename arg', () => {
	for (const filename of [{}, [], 0, null, undefined]) {
		// @ts-ignore
		assert.throws(() => parseNative(filename), undefined, `filename type: ${typeof filename}`);
	}
	// @ts-ignore
	assert.throws(() => parseNative(), undefined, `filename type: undefined`);

	assert.not.throws(() => parseNative('str'), undefined, `filename type: string`);
});
...
test('should resolve with empty result when filename is a tsconfig.json that does not exist and option is set', () => {
	const notExisting = path.resolve(os.homedir(), '..', 'tsconfig.json'); // outside of user home there should not be a tsconfig
	try {
		const result = parseNative(notExisting, { resolveWithEmptyIfConfigNotFound: true });
		assert.equal(
			result,
...
test('should resolve with expected for valid tsconfig.json', async () => {
	...
	for (const filename of samples) {
		...
		try {
			const actual = parseNative(filename);
...
test('should work with cache', async () => {
	...
	for (const filename of samples) {
		try {
			...
			const actual = parseNative(filename, { cache });
			...
			const reparsedResult = parseNative(filename, { cache });
			...
			if (filename.endsWith('.ts')) {
				...
				const reparsedByResultFilename = parseNative(actual.tsconfigFile, { cache });
				assert.is(
			...
			cache.clear();
			const afterClear = parseNative(filename, { cache });
...
test('should resolve with tsconfig that works when transpiling', async () => {
	...
	for (const filename of samples) {
		try {
			const { tsconfig } = parseNative(filename);
...
test('should prevent typescript file scanning when ignoreSourceFiles: true is set', async () => {
	...
	for (const filename of samples) {
		try {
			...
			expected.files = [];
			expected.include = [];
			const actual = parseNative(filename, { ignoreSourceFiles: true });
